...
function index(req, res) {
    var lang = req.lang();
    var _a = this.db, Application = _a.Application, Team = _a.Team, User = _a.User;
    //récupération 
    var id = req.user.id;
    User.findOne({
        where: { id: id }
        ...
        u.getTeams()
            ...
            ts.forEach(function (e, i) {
                e.getApplication().then(function (r) {
                    var re = r.toJSON();
                    re['user_role'] = e.role;
                    ifs.push(re);
                    if (i == ts.length - 1) {
            ...
            .catch(function (e) { return res.json([1]); });
    })
        .catch(function (e) { return res.json([3]); });
}
exports.index = index;
...
function redirect(req, res) {
    ...
    var redirect_uri = site.urlapp + '/application/infusionsoft/redirect?id=' + id;
...
function reauthorize(req, res) {
    ...
    var _b = req.params, type = _b.type, id = _b.id;
    Application.findOne({
        where: { id: id }
    })
        .then(function (I) {
        if (type == 'infusionsoft') {
            var redirect_uri = site.urlapp + '/application/infusionsoft/redirect?id=' + I.id;
            ...
            var redirect_uri = site.urlapp + '/application/trello/redirect/' + I.id;
            var url = 'https://trello.com/1/authorize?expiration=30days&name=NoteVocal&scope=read,write&response_type=token&key=' + site.trelloKey + '&return_url=' + encodeURIComponent(redirect_uri);
            return res.json({ success: url });
        }
    })
        .catch(function (e) { return res.send(lang['InfusionsoftNull']); });
...
function create(req, res) {
    ...
    var _a = this.db, Application = _a.Application, User = _a.User, Team = _a.Team;
    //récupération 
    var _b = req.body, name = _b.name, compteId = _b.compteId, type = _b.type;
    ...
    Application.create({ name: name, unique: unique, compteId: compteId, type: type, url: '' })
        .then(function (I) {
                    ...
                    newteam.setApplication(I)
                        ...
                        newteam.setUser(user)
                            ...
                            if (type == 'infusionsoft') {
                                ...
                                var redirect_uri = site.urlapp + '/application/trello/redirect/' + I.id;
                                var url = 'https://trello.com/1/authorize?expiration=30days&name=NoteVocal&read,write=read&response_type=token&key=' + site.trelloKey + '&return_url=' + encodeURIComponent(redirect_uri);
                                return res.json({ success: url });
                            }
                        })
                            .catch(function (e) {
