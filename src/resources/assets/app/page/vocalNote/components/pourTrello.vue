	...
	<div>
		...
		<b-modal v-model="newpour" hide-footer>
            <template slot="modal-title">
                <span>Edition de 'POUR' de trello</span>
            </template>
	                    ...
	                    <b-form-group>
	                        <b-input-group>
	                            ...
	                            <b-row style="width: 100%;">
	                                ...
	                                <b-col sm="4"><label for="name">{{$lang('externalOptionTrelloCardId')}} : </label></b-col>
	                           	</b-row>
	                            <b-row style="width: 100%;">
	                                <b-col sm="8">
	                                    <b-form-group>
	...
	export default {
        ...
        methods : {
        	onSubmit : async function () {
        		...
        		if( this.form.name && this.form.appId && this.form.cardId  ){
                    ...
                    let add = await fetch(url , {
                        ...
                        body: JSON.stringify({ type : 'trello' , name : this.form.name , appId : this.form.appId , cardId : this.form.cardId })
                    }) ;
        	...
        	async poursListe(){
                ...
                if ( find.ok ) { 
                    ...
                    if ( data.length !== undefined ) {
                        return this.pours = data.filter( e => e.type ==='trello' );
                    }
                }
                return this.pours = []; 
            ...
            async findallteams( ){
                ...
                let url = window.urlapplication + '/team/application/'+ this.trello  ;
                let find = await fetch( url )  ; 
                if ( find.ok ) { 
                    let data = await find.json() ; 
            ...
            async findallcards( ){
                ...
                let url = window.urlapplication + '/trello/lists/'+ this.trello  ; 
                let find = await fetch( url )  ; 
                if ( find.ok ) { 
                    let data = await find.json() ; 
                    ...
                    this.optionsCards = optionsCards; 
                }
                return [] ; 
